...
// 扩展单词数据
const expandedWords = [...baseWords];

// 生成更多单词
const wordTemplates = [
    { base: "Happy", variations: ["Joyful", "Cheerful", "Delighted", "Gleeful", "Merry"] },
    { base: "Good", variations: ["Great", "Excellent", "Perfect", "Superb", "Outstanding"] },
    { base: "Hello", variations: ["Hi", "Hey", "Greetings", "Salutations", "Welcome"] },
    { base: "Goodbye", variations: ["Bye", "Farewell", "See you", "Take care", "Adieu"] },
    { base: "Thank", variations: ["Thanks", "Thankful", "Grateful", "Appreciative", "Thank you"] },
    { base: "Sorry", variations: ["Apologies", "Pardon", "Excuse me", "Regret", "Sorry about that"] },
    { base: "Please", variations: ["Kindly", "Would you", "Could you", "May I", "If you please"] },
    { base: "Yes", variations: ["Yeah", "Sure", "Absolutely", "Definitely", "Of course"] },
    { base: "No", variations: ["Nope", "Negative", "Not at all", "Absolutely not", "No way"] },
    { base: "OK", variations: ["Okay", "Alright", "Fine", "Good", "Perfect"] }
];
...
for (let i = 0; i < 900; i++) {
    ...
    // 生成随机的句子和翻译
    const sentences = [
        `${variation}! How are you?`,
        `I'm ${variation}!`,
        `${variation}, my friend!`,
        `Have a ${variation} day!`,
        `${variation} to meet you!`
    ];
    ...
    const homophoneSentences = [
        `${variation.toLowerCase()}! 好啊油?`,
        `爱姆${variation.toLowerCase()}!`,
        `${variation.toLowerCase()}, 麦夫瑞恩德!`,
        `海夫啊${variation.toLowerCase()} dei!`,
        `${variation.toLowerCase()} to 米特油!`
    ];
    
    const sentence = sentences[Math.floor(Math.random() * sentences.length)];
    const translation = translations[Math.floor(Math.random() * translations.length)];
    const homophoneSentence = homophoneSentences[Math.floor(Math.random() * homophoneSentences.length)];
    
    // 简单的音标生成（实际应用中应该使用真实音标）
    const phonetic = `/${variation.toLowerCase()}/`;
    
    // 简单的谐音生成（实际应用中应该使用真实谐音）
    const homophone = variation.toLowerCase();
...
if (match) {
    const newGreetingsContent = expandedWords.map(word => {
    ...
    console.log('成功添加1000个日常问候单词到word-data.js');
} else {
